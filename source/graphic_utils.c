...
static void s_clip_se_rect_to_bounding_rect(Rect* rect, const Rect* bounding_rect);
static void s_clip_se_rect_to_screenblock(Rect* rect);
static void s_clip_se_rect_within_step_of_full_screen_vert(Rect* se_rect, int direction);
...
void main_bg_se_clear_rect(Rect se_rect)
{
    ...
    s_clip_se_rect_to_screenblock(&se_rect);
...
{
    s_bg_se_copy_or_move_rect_1_tile_vert(MAIN_BG_SBB, se_rect, direction, false);
}

void bg_se_move_rect_1_tile_vert(u16 bg_sbb, Rect se_rect, enum ScreenVertDir direction)
{
    s_bg_se_copy_or_move_rect_1_tile_vert(MAIN_BG_SBB, se_rect, direction, true);
}

void main_bg_se_copy_rect_1_tile_vert(Rect se_rect, enum ScreenVertDir direction)
{
    s_main_bg_se_copy_or_move_rect_1_tile_vert(se_rect, direction, false);
}

void main_bg_se_move_rect_1_tile_vert(Rect se_rect, enum ScreenVertDir direction)
{
    s_main_bg_se_copy_or_move_rect_1_tile_vert(se_rect, direction, true);
}
...
{
    s_clip_se_rect_to_screenblock(&se_dest_rect);
    ...
    // Copy the corners
    s_main_bg_se_expand_3x3_copy_corners(&se_dest_rect, &src_top_left_pnt, dest_rect_width, dest_rect_height);

    // Copy top and bottom sides
    s_main_bg_se_expand_3x3_copy_top_bottom(&se_dest_rect, &src_top_left_pnt, dest_rect_width);

    // Copy left and right sides
    s_main_bg_se_expand_3x3_copy_left_right(&se_dest_rect, &src_top_left_pnt, dest_rect_width, dest_rect_height);
...
static void s_clip_se_rect_to_screenblock(Rect* rect)
{
    s_clip_se_rect_to_bounding_rect(rect, &FULL_SCREENBLOCK_RECT);
}
...
static void s_clip_se_rect_within_step_of_full_screen_vert(Rect* se_rect, int direction)
{
    ...
    s_clip_se_rect_to_bounding_rect(se_rect, &bounding_rect);
}
...
static void s_bg_se_copy_or_move_rect_1_tile_vert(u16 bg_sbb, Rect se_rect, int direction, bool move)
{
    ...
    // Clip to avoid read/write overflow of the screenblock
    s_clip_se_rect_within_step_of_full_screen_vert(&se_rect, direction);
...
static void s_main_bg_se_copy_or_move_rect_1_tile_vert(Rect se_rect, int direction, bool move)
{
    s_bg_se_copy_or_move_rect_1_tile_vert(MAIN_BG_SBB, se_rect, direction, move);
}
